...
class TestBooksCollector:
    ...
    def test_get_books_for_children_two_books(self, books_data, expected_result): # Проверяем фильтрацию книг по жанру, чтобы возвращать только те книги, которые подходят для детей
...
def test_add_new_book_add_one_books_true (test_books_collector): #Проверяем добавление новой книги
    test_books_collector.add_new_book("Книга 1")
    assert "Книга 1" in test_books_collector.get_books_genre()

def test_add_new_book_repeated_name_false(test_books_collector): # Проверяем, что не добавляется книга с повторным названием
    test_books_collector.add_new_book("Книга 1")
    assert len(test_books_collector.books_genre) == 1, "Книга не должна добавиться повторно"

def test_add_new_book_long_name_false(test_books_collector):   # Проверяем, что книга с длинным названием не добавляется
...
def test_set_book_genre_book_collection_fantastic(test_books_collector): # проверяем, корректно ли устанавливается жанр книги
    test_books_collector.add_new_book("Книга 1")
    test_books_collector.set_book_genre("Книга 1", "Фантастика")
    assert test_books_collector.get_book_genre("Книга 1") == "Фантастика"

def test_set_book_genre_defunct_book_false(test_books_collector):# Проверяем, что нельзя установить жанр для несуществующей книги
...
def test_set_book_genre_defunct_genre_false(test_books_collector): # Проверяем, что нельзя установить несуществующий жанр
...
def test_get_books_with_specific_genre_set_genre_true(test_books_collector):   # проверяем, добавление книг и назначение им жанров, а также может ли корректно возвращать список книг по заданному жанру
    test_books_collector.add_new_book("Книга 1")
    test_books_collector.set_book_genre("Книга 1", "Фантастика")
...
def test_add_remove_favorites_cannot_add_again(test_books_collector): # Проверяем, что нельзя добавить книгу повторно в избранное
...
def test_add_remove_favorites_delete_books(test_books_collector): # Удаляем книгу из избранного
...
def test_get_book_genre_by_name(test_books_collector):  # проверяем получение жанр книги по её имени
    test_books_collector.add_new_book("Книга 1")
    test_books_collector.set_book_genre("Книга 1", "Фантастика")
    test_books_collector.add_new_book("Книга 2")
...
def test_get_list_of_favorites_books_true(test_books_with_favorites): # Проверяем получение списка избранных книг
